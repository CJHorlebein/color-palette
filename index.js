...
class App extends Component {
    ...
    randomColors(){
        let newColors = this.state.colors.map(color => {
            ...
            for(let i = 0; i < 6; i++){
                randColor.push(Math.floor(Math.random() * 16).toString(16).toUpperCase());
            }
            return {color: randColor.join(''), locked: color.locked};
        })
        ...
        return (
            <div>
                <div className="text-center bg-dark fixed-top">
                    <button onClick={() => this.randomColors()} className="btn btn-secondary m-1">RANDOMIZE COLORS</button>
                </div>
                <div className="w-100 d-flex" style={{height: "100vh"}}>
                    {this.state.colors.map((color, i) => (
                            <div key={i} style={{backgroundColor: color.color}} className="w-100 d-flex flex-column align-items-center justify-content-center">
                                <h1>{color.color}</h1>
                                <button onClick={() => this.lockColor(i)} className="btn btn-dark">{color.locked ? 'locked' : 'unlocked'}</button>
                            </div>
                        )
                    )}
                </div>
